                            ...
                            [
                                dbc.CardBody(
                                    [
                                        ...
                                        html.Label(
                                            "Buscar en citas:", className="fw-bold"
                                        ),
                                        dcc.Input(
                                            id="entrada-busqueda",
                                            type="text",
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H5(
                                            "📅 Distribución por siglo de nacimiento",
                                            className="mb-3",
                                        ),
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H5(
                                            "💬 Citas filtradas (mostrando hasta 20)",
                                            className="mb-3",
                                        ),
...
def actualizar_dashboard(texto_busqueda, autor_seleccionado, etiqueta_seleccionada):
    ...
    if etiqueta_seleccionada != "all":
        df_filtrado = df_filtrado[
            df_filtrado["tags"].apply(lambda x: etiqueta_seleccionada in x)
        ]
    ...
    if total_citas > 0 and len(df_etiquetas_filtrado) > 0:
        ...
        conteo_etiquetas = (
            df_etiquetas_filtrado[df_etiquetas_filtrado["tags"].str.strip() != ""][
                "tags"
            ]
            .value_counts()
            .head(15)
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.Span(
                                    f" • {fila['author_birthdate'].strftime('%d/%m/%Y')}",
                                    className="text-muted ms-2",
                                ),
                                html.Span(
                                    f" • {fila['author_birthplace']}",
                                    className="text-muted ms-2",
                                ),
                            ]
                        ),
